...
def abort_if_exists(user_id, category_name=None, category_id=None, recipe_name=None):
    ...
    if category_name:
        ...
        if category_name in categorylist:
            abort(400, {"status" : "Bad request","message" : "Category already exists"})
    if recipe_name:
        category = Category.get_by_id(category_id)
        if category.user_id == user_id:
            if Recipe.recipe_exists(category_id, recipe_name):
                abort(400, {"status" : "Bad request","message" : "Recipe already exists"})
        else:
            abort(403, {"status" : "Access denied","message" : "Category does not belong to this user"})
